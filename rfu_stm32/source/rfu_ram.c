    ...
    case process_command:
      ...
      if (opcode == o_info) {
        ...
        if (opcode == o_crc) {
          ...
          uint32_t checksum_data = read32(&in_buffer[8]);
          /* calc */
          uint8_t checksum = 0;
          uint8_t *check = (uint8_t *)start_address;
          for (uint32_t i = 0; i < length; ++i) {
            CRC8_NEXT(checksum, *check); /*crc push*/
            check++;
          }

          if ( checksum_data == checksum ) {
             send_u32(nt_address, o_crc, checksum);
             protocol_state = rx_start_symbol;
             break;
          } else {
            // do not answer
             protocol_state = rx_start_symbol;
             break;
            // send_ec(nt_address, o_crc, 0xFF); //-1 error crc check
          }
        } else if (opcode == o_erase) {
          // todo add custom flags for pages erasure
          // erase flash
          uint8_t ec = (FLASH_COMPLETE == RFU_FLASH_EraseAllPages()) ? 0 : 4;
...
/* check crc32 of data_ptr length */
          ...
          if (!IS_FLASH_ADDRESS(start_address) &&
              (!IS_FLASH_ADDRESS(start_address +
                                 length))) { // address range error:
            // send_ec(nt_address, o_write, 2);
            protocol_state = rx_start_symbol;
            break;
          }
          ...
          for (uint32_t address = start_address;
               address < start_address + length; address += sizeof(uint32_t)) {
            uint32_t data = read32(data_ptr);
            FLASH_Status st = RFU_FLASH_ProgramWord(address, data);
